...
;;bg; This should be a class
(define (make-definite-integrator #:integrand [integrand #f]
                                  #:lower-limit [lower-limit #f]
                                  #:upper-limit [upper-limit #f]
                                  #:allowable-error [allowable-error 1.0e-10]
                                  #:method [method 'open])
  (lambda ( message . rest)
    (case message
      [(integral)
      ...
      [(integrand)
       integrand]
      [(set-integrand!)
       (set! integrand (car rest))]
      [(lower-limit)
       lower-limit]
      [(set!-lower-limit)
       (set! lower-limit (car rest))]
      [(upper-limit)
       upper-limit]
      [(set-upper-limit!)
       (set! upper-limit (car rest))]
      [(error)
       allowable-error]
      [(set-error!)
       (set! allowable-error (car rest))]
      [(method)
       method]
      [(set-method!)
       (set! method (car rest))]
      [else
       (error 'integrator (format "Unknown message ~a" message))])))
